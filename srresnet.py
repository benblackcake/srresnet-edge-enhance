...
class Srresnet:
    ...
    def forward(self,x_LL, x_edge):
        with tf.variable_scope('forward_branch',reuse=tf.AUTO_REUSE) as scope:
            ...
            y_pred_concat_R = tf.concat([tf.cast(tf.expand_dims(y_pred[:,:,:,0],axis=-1),tf.float32),x_edge[:,:,:,0:3]], axis=-1)
            y_pred_concat_G = tf.concat([tf.cast(tf.expand_dims(y_pred[:,:,:,1], axis=-1),tf.float32),x_edge[:,:,:,3:6]], axis=-1)
            y_pred_concat_B = tf.concat([tf.cast(tf.expand_dims(y_pred[:,:,:,2], axis=-1),tf.float32),x_edge[:,:,:,6:9]], axis=-1)

            y_pred_concat = tf.concat([y_pred_concat_R, y_pred_concat_G, y_pred_concat_B],axis=-1)

            return x_LL, x_edge, y_pred, y_pred_concat
    ...
    def loss_function(self, y_A, y_A_pred, y_BCD, y_BCD_pred, hr, sr_pred, sr_pred_concat):

        if self.use_gan:
            return self._content_loss(y_A, y_A_pred, y_BCD, y_BCD_pred, hr, sr_pred) +1e-3*self._adversarial_loss(sr_pred_concat)
            print('Using GAN')
...
class SRGanDiscriminator:
    ...
    def forward(self, x):
        ...
        with tf.variable_scope('discriminator') as scope:
            ...
            x.set_shape([None, self.image_size, self.image_size, 12])
